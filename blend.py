    ...
    for i in range(n_iter):
        ...
        for run, files in runs.items():
            ...
            if not predict:
                ...
                kappa_scr.append(util.kappa(labels[te],y_pred))
                print("confusion matrix")
                print(confusion_matrix(labels[te], y_pred))
                precision_scr = precision_score(labels[te],y_pred)
                print('precision score: ' + str(precision_scr))
                precision.append(precision_scr)
                recall_scr = recall_score(labels[te],y_pred)
                print('recall or sensitivity score: ' + str(recall_scr))
                recall.append(recall_scr)
 

                # keep all metrics score in a csv file
                scores = {'kappa':kappa_scr,'precision':precision,'recall':recall}
                metrics_score = pd.DataFrame.from_dict(scores)
                metrics_score.to_csv('val_score_1.csv')
            else:
